...
// PŘIDÁNÍ FILMU DO POLE

const harryPotter = {
	id: 'princ-dvoji-krve',
	nazev: 'Harry Potter a Princ dvojí krve',
	plakat: {
		url: 'https://static.posters.cz/image/750webp/104623.webp',
		sirka: 420,
		vyska: 592,
	},
	ochutnavka: 'Šestý díl filmové série o čarodějnickém učni.',
	popis:
...
const dnyOdPremiery = dayjs().diff(vybranyFilm.premiera, 'days');
// console.log(dnyOdPremiery);

if (dnyOdPremiery > 2) {
	premiera.innerHTML = `Premiéra <strong>${datumPremiery}</strong>, což bylo před ${dnyOdPremiery} dny.`;
}
else if (dnyOdPremiery == 1) {
	premiera.innerHTML = `Premiéra <strong>${datumPremiery}</strong>, což bylo před ${dnyOdPremiery} dnem.`;
}
else if (dnyOdPremiery == 0) {
	premiera.innerHTML = `Premiéra <strong>${datumPremiery}</strong>, což je DNES!`;
}
else if (dnyOdPremiery < -4) {
	premiera.innerHTML = `Premiéra <strong>${datumPremiery}</strong>, což bude za ${Math.abs(dnyOdPremiery)} dní.`;
}
else if (dnyOdPremiery < -1) {
	premiera.innerHTML = `Premiéra <strong>${datumPremiery}</strong>, což bude za ${Math.abs(dnyOdPremiery)} dny.`;
}
else if (dnyOdPremiery < 0) {
	premiera.innerHTML = `Premiéra <strong>${datumPremiery}</strong>, což bude za ${Math.abs(dnyOdPremiery)} den.`;
}
...
stars.forEach((star, index) => {
	star.addEventListener('click', () => {
		const oznacenaStar = index + 1;
		posledniKlik = oznacenaStar; // Kdykoliv uživatel na nějakou hvězdičku klikne, poznamenejte si bokem, kolikátá to byla.
		zvyrazneneStars(oznacenaStar);
	});
